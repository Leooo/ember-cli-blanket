...
module.exports = Reporter.extend({
  ...
  transform: function(coverageData) {
    ...
    var threshold = this.options.thresholdPassTests;
    if (threshold && threshold > output.coverage.total.percentage) {
      console.log('not ok: blanket total test coverage is below thresholdPassTests: ' + output.coverage.total.percentage);
    }
